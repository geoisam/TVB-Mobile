...
object LunarUtil {
    ...
    fun getWeek(): String {
        val timeNow = ZonedDateTime.now(ZoneId.systemDefault())
        val solar = Solar.fromYmd(timeNow.year, timeNow.monthValue, timeNow.dayOfMonth)
    ...
    fun getShiChen(): String {
        val timeNow = ZonedDateTime.now(ZoneId.systemDefault())
        val lunar = Lunar.fromYmdHms(timeNow.year, timeNow.monthValue, timeNow.dayOfMonth, timeNow.hour, timeNow.minute, timeNow.second)
    ...
    fun getNextFestival(
        ...
        for (i in 0 until maxDays) {
            ...
            if (nearestJieQi == null) {
                ...
                if (!jieQi.isNullOrBlank()) {
                    nearestJieQi = FestivalModel(name = jieQi, date = date, weekday = solar.weekInChinese, daysLeft = i.toLong())
                }
            }
        ...
        nearestFestivalDay?.let { days ->
            ...
            val javaDate =
                Date.from(date.atStartOfDay(zone).toInstant())
            val solar = Solar.fromDate(javaDate)

            nearestFestivals.forEach { name ->
                result.add(FestivalModel(name = name, date = date, weekday = solar.weekInChinese, daysLeft = days))
            }
        }
